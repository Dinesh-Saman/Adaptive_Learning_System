...
// ── 5 Question Items for Activity 4: මැජික් පුවරුවේ අකුරු ලියමු ──
const ACTIVITY4_ITEMS = [
  {
    id: 1,
    num: 1,
    letter: 'එ',
    phoneme: 'එ',
    wordExample: 'එළදෙන (Cow)',
    voiceText: 'එ යන අකුර ලියන්න.',
  ...
  },
  {
    ...
    letter: 'උ',
    phoneme: 'උ',
    wordExample: 'උදෑසන (Morning)',
    voiceText: 'උ යන අකුර ලියන්න.',
  ...
  },
  {
    ...
    letter: 'ප',
    phoneme: 'ප',
    wordExample: 'පොත (Book)',
    voiceText: 'ප යන අකුර ලියන්න.',
  ...
  },
  {
    ...
    letter: 'ල',
    phoneme: 'ල',
    wordExample: 'ලස්සන (Beautiful)',
    voiceText: 'ල යන අකුර ලියන්න.',
  ...
  },
  {
    ...
    letter: 'ඔ',
    phoneme: 'ඔ',
    wordExample: 'ඔරලෝසුව (Clock)',
    voiceText: 'ඔ යන අකුර ලියන්න.',
...
// ── Strict Stroke Skeleton Evaluator for Magic Board ──
function evaluateMagicBoardTracing(canvas, targetLetter) {
  ...
  refCtx.font = 'normal 120px "Iskoola Pota", "Noto Sans Sinhala", sans-serif';
  refCtx.textAlign = 'center';
  refCtx.textBaseline = 'alphabetic';
  refCtx.strokeText(targetLetter, w / 2, 125);
  ...
  if (totalRefPixels === 0 || totalDrawnPixels < 160) {
    return {
      completionPercent: 0,
      accuracyPercent: 0,
      isComplete: false,
...
// ── Single Magic Board Writing Slate Component ──
function MagicBoardSlate({ item, isDone, onComplete, onIncomplete }) {
  ...
  const handleStart = (e) => {
    ...
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ctx.strokeStyle = '#16A34A'; // Magic Green ink
  ...
  const handleEnd = (e) => {
    ...
    const { isComplete, missingReason } = evaluateMagicBoardTracing(canvas, item.letter);
  ...
  const handlePlayVoice = (e) => {
    e.stopPropagation();
    playSound('click');
    speakSinhala(`"${item.letter}" යන අකුර ලියන්න.`);
  };
  ...
  return (
    <div className={`relative bg-white rounded-3xl p-4 shadow-lg border-4 ${item.border} flex flex-col justify-between transition-all hover:shadow-xl`}>
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>"{item.letter}" යන අකුර ලියන්න.</span>
          </button>
        </div>
      ...
      {/* Main Tracing Blackboard Canvas */}
      <div className="w-full h-36 md:h-40 bg-gradient-to-b from-slate-50 to-white rounded-2xl border-2 border-dashed border-slate-300 relative flex items-center justify-center overflow-hidden shadow-inner">
        ...
        {/* Thin Dashed Guideline Letter */}
        <svg viewBox="0 0 280 180" className="w-full h-full pointer-events-none select-none">
          <text
            x="140"
            y="125"
            textAnchor="middle"
...
// ── Main Grade 2 Level 1 Activity 4 Component ──
export default function SinhalaGrade2Level1Act4({ onExit }) {
  ...
  const totalPages = 2;
  const pageItems = currentPage === 0 ? ACTIVITY4_ITEMS.slice(0, 4) : ACTIVITY4_ITEMS.slice(4);

  // Play instructions on load
  useEffect(() => {
    const timer = setTimeout(() => {
      speakSinhala('මැජික් පුවරුවේ ශබ්දය අසලා අකුර ලියන්න.');
    }, 400);
  ...
  const handleSlateCompleted = (item) => {
    ...
    speakSinhala(`නියමයි! "${item.letter}" අකුර නිවැරදියි!`);
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-indigo-300 via-purple-100 to-pink-200 flex items-center justify-center p-4 font-sinhala">
        <div className="max-w-xl w-full bg-white rounded-[3rem] p-8 shadow-2xl border-8 border-yellow-300 text-center">
          ...
          <p className="text-slate-600 text-lg mb-6">ඔබ මැජික් පුවරුවේ සියලු අකුරු සාර්ථකව ලියා අවසන් කළා!</p>
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              <span>🔓 LEVEL 2 — Activity 1 (වචන ගොඩනගමු)</span>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-sky-400 via-sky-200 to-emerald-200 font-sinhala select-none relative overflow-x-hidden pb-6">
      ...
      <div className="max-w-5xl mx-auto px-4 py-3 relative z-10 flex flex-col min-h-screen justify-between">
        ...
        <div className="flex items-center justify-between gap-3 mb-2">
          ...
          <div className="flex-1 max-w-xl bg-gradient-to-r from-purple-700 via-indigo-600 to-purple-700 text-white py-2.5 px-6 rounded-full shadow-lg border-2 border-yellow-300 text-center">
            <h1 className="text-xl md:text-2xl font-extrabold tracking-wide drop-shadow flex items-center justify-center gap-2">
              <span>✨</span>
              <span>Activity 4: මැජික් පුවරුවේ අකුරු ලියමු</span>
              <span>✨</span>
            </h1>
          </div>
        ...
        <div className="relative max-w-3xl mx-auto w-full my-2">
          <div className="bg-white/95 backdrop-blur-md rounded-full py-2.5 px-6 shadow-md border-2 border-dashed border-purple-300 flex items-center gap-3">
            ...
            <p className="text-lg md:text-xl font-bold text-slate-800">
              <span className="text-purple-700 font-extrabold">ශබ්දය අසලා</span> අකුර <span className="text-emerald-600 font-extrabold underline">ලියන්න.</span>
            </p>
          </div>
        ...
        {/* ── 2x2 GRID OF MAGIC BOARD SLATES ── */}
