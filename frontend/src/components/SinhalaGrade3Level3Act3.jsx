...
// ── Sinhala Riddles Data (තේරවිලි තෝරා පිළිතුර ලියන්න) ──
const LEVEL3_ACT3_RIDDLES = [
  {
    id: 1,
    num: 1,
    lines: [
    ...
    correctAnswer: 'කොකා',
    meaning: 'Stork / Crane standing patiently in the paddy field looking for fish',
    imageEmoji: '🕊️🌾🐟',
    audioPrompt: 'තපස් රකිනවා, කෙතේ ඉන්නවා, මාළු සොයනවා, කියන්න මගේ නම? පිළිතුර කොකා.',
    options: [
      { id: 'opt_1_1', text: 'කොකා', isCorrect: true },
      { id: 'opt_1_2', text: 'ඉබ්බා', isCorrect: false },
      { id: 'opt_1_3', text: 'කපුටා', isCorrect: false },
    ]
  },
  {
    id: 2,
    ...
    lines: [
      'පිරිසිදු කම කරන අපේ',
      'රැවුල දිගයි ගෙයි මුල්ලේ',
      'කුණු දූවිලි අතුගා දමයි',
      'කියන්න මගේ නම.'
    ],
    correctAnswer: 'ඉදල',
    meaning: 'Broom / Sweeper in the corner of the house',
    imageEmoji: '🧹🏠✨',
    audioPrompt: 'පිරිසිදු කම කරන අපේ, රැවුල දිගයි ගෙයි මුල්ලේ, කුණු දූවිලි අතුගා දමයි, කියන්න මගේ නම? පිළිතුර ඉදල.',
    options: [
      { id: 'opt_2_1', text: 'ඉදල', isCorrect: true },
      { id: 'opt_2_2', text: 'කොස්ස', isCorrect: false },
      { id: 'opt_2_3', text: 'වරල', isCorrect: false },
    ]
  },
  {
    id: 3,
    num: 3,
    lines: [
      'උඩින් ගෙදර බිම යනකොට',
      'අතරමගදී වැටී තිබුණු',
      'මල් සුවඳයි ගෙඩි තිත්තයි',
      'තේරුවොත් ලොකු විස්තරයි.'
    ],
    correctAnswer: 'කරවිල',
    meaning: 'Bitter gourd with sweet-smelling flowers and bitter fruit',
    imageEmoji: '🥒🌿🌸',
    audioPrompt: 'මල් සුවඳයි ගෙඩි තිත්තයි, තේරුවොත් ලොකු විස්තරයි. පිළිතුර කරවිල.',
    options: [
      { id: 'opt_3_1', text: 'කරවිල', isCorrect: true },
      { id: 'opt_3_2', text: 'වම්බටු', isCorrect: false },
      { id: 'opt_3_3', text: 'තිත්තලබු', isCorrect: false },
    ]
  },
  {
    id: 4,
    ...
    lines: [
      ...
      'ඒක ඇතුළේ කැරකෙනවා',
      'ගේ දොර වටේ එනවා',
      'තේරුණොතින් පඩි දෙනවා.'
    ],
    correctAnswer: 'යතුර',
    meaning: 'Key that rotates inside the keyhole',
    imageEmoji: '🔑🚪🔒',
    audioPrompt: 'හුලං බඩේ රිංගනවා, ඒක ඇතුළේ කැරකෙනවා. පිළිතුර යතුර.',
    options: [
      { id: 'opt_4_1', text: 'යතුර', isCorrect: true },
      { id: 'opt_4_2', text: 'අගුල', isCorrect: false },
      { id: 'opt_4_3', text: 'ඉබ්බා', isCorrect: false },
    ]
  },
  {
    id: 5,
    ...
    lines: [
      'කටු ඇතුලේ කිරි පැණි',
      'ගසේ උඩම ඉන්නවා',
      'ගෙඩි කඩලා වතුර බොනවා',
      'කියන්න මගේ නම.'
    ],
    correctAnswer: 'පොල්',
    meaning: 'Coconut on top of the tree with water inside',
    imageEmoji: '🥥🌴💧',
    audioPrompt: 'ගසේ උඩම ඉන්නවා, ගෙඩි කඩලා වතුර බොනවා. පිළිතුර පොල්.',
    options: [
      { id: 'opt_5_1', text: 'පොල්', isCorrect: true },
      { id: 'opt_5_2', text: 'තැබිලි', isCorrect: false },
      { id: 'opt_5_3', text: 'අඹ', isCorrect: false },
    ]
  }
];
...
// ── Ruled Word Writing Slate ──
function RiddleWordWritingSlate({ targetWord }) {
    ...
    <div className="w-full mt-3 p-3 bg-slate-50 rounded-2xl border-2 border-dashed border-sky-300">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs font-bold text-sky-900 flex items-center gap-1.5">
          <span>✍️</span>
          <span>පිළිතුර තිත් ඉරි මත ලියන්න (Write answer on dotted guide):</span>
        </span>
        <button
          onClick={handleClear}
      ...
      <div className="relative w-full h-16 bg-white rounded-xl border border-slate-200 overflow-hidden flex items-center justify-center">
        ...
        <svg viewBox="0 0 320 50" className="w-full h-full pointer-events-none select-none">
          <text
            x="160"
            y="35"
            textAnchor="middle"
...
// ── Main Grade 3 Level 3 Activity 3 Component ──
export default function SinhalaGrade3Level3Act3({ onExit }) {
  ...
  const currentQ = LEVEL3_ACT3_RIDDLES[currentIndex];

  useEffect(() => {
    setSelectedOptionId(null);
    setIsAnswered(false);
    const timer = setTimeout(() => {
      speakSinhala(currentQ.lines.join(', '));
    }, 400);
  ...
  const handleSelectOption = (opt) => {
    ...
    if (opt.isCorrect) {
      ...
      speakSinhala(`විශිෂ්ටයි! තේරවිල්ලේ පිළිතුර වන්නේ ${opt.text}. දැන් වචනය ලියන්න.`);
    } else {
      playSound('wrong');
  ...
  const handleNext = () => {
    playSound('click');
    if (currentIndex < LEVEL3_ACT3_RIDDLES.length - 1) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-sky-400 via-purple-100 to-emerald-200 flex items-center justify-center p-4 font-sinhala">
        <div className="max-w-xl w-full bg-white rounded-[3rem] p-8 shadow-2xl border-8 border-yellow-300 text-center animate-bounce-short">
          ...
          <p className="text-slate-600 text-lg mb-2">ඔබ Level 3 Activity 3 (තේරවිලි) සාර්ථකව අවසන් කළා!</p>
    ...
    <div className="min-h-screen bg-gradient-to-b from-sky-400 via-sky-100 to-emerald-200 font-sinhala select-none relative overflow-x-hidden flex flex-col justify-between pb-6">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex-1 max-w-lg bg-gradient-to-r from-purple-700 via-indigo-700 to-purple-800 text-white py-2 px-6 rounded-full shadow-lg border-2 border-yellow-300 text-center">
            <h1 className="text-sm md:text-base font-black tracking-wide text-yellow-200 drop-shadow">
              Activity 3: තේරවිලි තෝරා පිළිතුර ලියන්න
            </h1>
          </div>
          ...
          <div className="bg-white/95 backdrop-blur-md rounded-full py-2 px-6 shadow-md border-2 border-purple-300 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  playSound('click');
                  speakSinhala('පහත තේරවිල්ල කියවා නිවැරදි පිළිතුර තෝරා ලියන්න.');
                }}
              ...
              <p className="text-xs sm:text-sm md:text-base font-bold text-slate-800">
                📜 පහත <span className="text-purple-700 font-extrabold underline">තේරවිල්ල කියවා</span> නිවැරදි පිළිතුර තෝරා ලියන්න.
              </p>
            </div>
      ...
      <div className="max-w-4xl mx-auto w-full px-4 my-3 flex-1 flex flex-col justify-center">
        <div className="bg-white/95 backdrop-blur-md rounded-[2.5rem] p-6 md:p-8 shadow-2xl border-4 border-purple-200 flex flex-col gap-4 relative overflow-hidden">
          ...
          {/* Top Row: Question Step Badge + Audio Button */}
          <div className="flex items-center justify-between border-b border-purple-100 pb-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-bold text-slate-500">තේරවිල්ල {currentQ.num} / 5</span>
            </div>
          ...
          {/* Middle: Left Poetic Riddle Box + Right 3 Radio Options */}
          <div className="flex flex-col sm:flex-row items-center justify-between gap-5 my-1">
            
            {/* Left Riddle Poem Card (Matching Screenshot Poetic Style) */}
            <div className="flex-1 w-full bg-gradient-to-tr from-amber-50 via-orange-50 to-yellow-100 rounded-3xl p-4 sm:p-5 border-3 border-amber-300 shadow-md flex flex-col justify-between min-h-[160px]">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-black text-amber-900 uppercase tracking-widest flex items-center gap-1.5">
                  <span>📜</span>
                  <span>තේරවිලි කවිය:</span>
                </span>
                <span className="text-2xl drop-shadow-sm">{currentQ.imageEmoji}</span>
              </div>

              <div className="space-y-1 my-1 text-center sm:text-left">
                {currentQ.lines.map((line, lIdx) => (
                  <p key={lIdx} className="text-sm sm:text-base md:text-lg font-black text-slate-800 leading-snug">
                    {line}
                  </p>
                ))}
              </div>

              <span className="text-[10px] font-bold text-amber-800/80 mt-2 block text-center sm:text-left">
                💡 {currentQ.meaning}
              </span>
            </div>

            {/* Right: 3 Radio Option Cards */}
          ...
          {/* ── Bottom Ruled Handwriting Slate for Answer Word ── */}
          {isAnswered && (
            <div className="animate-fade-in">
              <RiddleWordWritingSlate targetWord={currentQ.correctAnswer} />
            </div>
          )}
